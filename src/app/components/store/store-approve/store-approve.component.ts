...
@Component({
  ...
  styleUrl: './store-approve.component.scss'
})
export class StoreApproveComponent implements OnInit {
  menuLabel2: string = 'สโตร์อนุมัติเบิกสินค้า > รายการเบิกวัสดุสิ้นเปลือง';
  ...
  loadWaitApprove(){
    this.rest.getStoreApprove().pipe(timeout(60000)).subscribe({
      next: (res: any) => {
        if (res.success) {
